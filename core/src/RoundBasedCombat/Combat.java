...
public class Combat {
    ...
    private List<CharacterUnit> group = new ArrayList<CharacterUnit>();

    private List<CombatTurnEntry> turnActions = new ArrayList<CombatTurnEntry>();

    public Combat(List<CharacterUnit> group, List<Enemy> enemies) {
    ...
    public List<Unit> getGroup(Unit unit) {
        if (group.contains(unit)) {
            return castToUnit(group.toArray());
        } else {
            return castToUnit(enemies.toArray());
        }
    }
    
    public List<Unit> castToUnit(Object ... group) {
   ...
   /* private List<Unit> castToUnit(List<Enemy> enemies) {
        List<Unit> tmp = new ArrayList<Unit>();
    ...
    public List<CharacterUnit> getPlayers() {
    	return group;
    }
    ...
    public AttackResult damage(Unit source, Unit target, Ability ability) {
        ...
        if (!target.hasStatus(UnitStatus.UNATTACKABLE) && !target.hasStatus(UnitStatus.DEAD)) {
            if (result.getResult().equals(AttackLanded.LANDED)) {
                ...
                ability.execute(this, source, target, result);
    ...
    private void queueEnemyAbilitys() {
        for (Enemy enemy : enemies) {
            for (CombatTurnEntry entry : enemy.getAbilitys(this, group)) {
                turnActions.add(entry);
            }
        }
    }
    ...
    public List<CombatAction> doRound() {
        ...
        updateBuffsInGroup(castToUnit(enemies.toArray()));
        updateBuffsInGroup(castToUnit(group.toArray()));
